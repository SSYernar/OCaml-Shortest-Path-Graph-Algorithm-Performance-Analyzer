...
let a_star graph =
  print_endline "Loading A* algorithm...";
  ...
  let heuristic = Array.make num_nodes infinity in
  ...
  let rec find_min_distance_node dist heuristic visited =
    let min_dist = ref infinity in
    let min_node = ref (-1) in
    for i = 0 to num_nodes - 1 do
      if not visited.(i) && (dist.(i) +. heuristic.(i)) < !min_dist then (
        min_dist := dist.(i) +. heuristic.(i);
        min_node := i
      )
    done;
    !min_node
  ...
  for _ = 0 to num_nodes - 1 do
    let u = find_min_distance_node dist heuristic visited in
    if u != -1 then (
      visited.(u) <- true;
